...
- Trimming `docs/agentdex-design.md`'s CLI documentation to architecture and rationale: the command list becomes the noun-verb tree, and the per-command behavioural spec (the selector rule, the per-command notes, the coverage-table fallthrough rows, the get-reporting exit bullets) is removed rather than rewritten. That spec duplicates the code and is the source of the drift this project corrects, so it is retired instead of re-synced.
...
The design doc has drifted only in its CLI-facing behavioural spec; the library API, detection engine, catalog schema, models.dev merge, caching, platforms, and start-migration sections are accurate against the code and are untouched. This project trims the drifted sections rather than re-syncing them: the command list (around 706), the selector-matching section (719-742), the per-command behaviour notes (744-825), the two `no` rows and the provider-fallthrough prose of the catalog/models.dev coverage section (676-700), the `get`-reporting per-case bullet in Output and exit codes (846-864), the Model resolution CLI-surface sentence (632-636), and the selector, model-resolution, and coverage bullets in the decisions log (1091-1114). The home-provider coverage rollup and its rationale stay; only the removed provider fallthrough leaves.
...
2. Bare noun. Invoking a noun group with no verb (`agentdex agents`) is a usage fault: exit 2, routed through the shared usage path. Under `--json` it emits the error envelope alone on stdout, its message naming the missing verb, and prints no help — the group help is a text-mode affordance only, so the JSON envelope stays parseable. In text mode it prints the group's help so the available verbs are discoverable, then exits 2 through the usage path.
...
5. Canonical ids. An agent's canonical id is its catalog kebab-case key (`claude-code`). A provider's canonical id is its models.dev provider id (`anthropic`). A model's canonical id is the composite `provider-id/model-id` (`anthropic/claude-opus-4-5`) — the composite the code already computes as `pid + "/" + key`. `models get` takes this composite and splits it on the first `/` only: the prefix up to that slash is the provider id, and the entire remainder is the model key, which may itself contain slashes. This is unambiguous because a models.dev provider id never contains a slash, while over half of real model keys do (for example `mixlayer/qwen/qwen3.5-122b-a10b` splits to provider `mixlayer`, model key `qwen/qwen3.5-122b-a10b`). A value with no `/` at all is a usage error (exit 2) that directs the user to `models list`.
...
10. `models list`. A browse over models. With no scope it lists models across all providers; the positional filter narrows by case-insensitive substring over model id and name. `--provider <id>` scopes the listing to one provider; `--agent <id>` scopes it to the named agent's providers. Scope composes with the filter. Every caller-supplied `--provider` id is validated against a reachable models.dev in both roles it plays here — the standalone direct scope (no `--agent`) and the enrichment set for an agnostic `--agent` — so an unknown id is a usage error (exit 2), matching `agents list`, never a silent empty listing. Preserve the existing agnostic and home-provider rules: an agnostic agent named by `--agent` requires `--provider`, and a home-provider agent rejects `--provider` as a usage error. Preserve newest-first ordering and the price footer. A models.dev outage with no cache is transient; structural drift is config.

11. `models get`. Exact single-model fetch by the composite id per Requirement 5: split the composite on the first `/` into the provider id and the (possibly slash-bearing) model key, fetch that provider, look up the model key in its models map, and render the existing single-model detail (the `modelRecord` fields, the price footer, `canonical_id` when the model has a real models.dev agnostic id). Compute `canonical_id` from the full input composite as the `Catalog.Models` lookup key, matching how the code already derives it, rather than re-joining the split parts. No fuzzy matching. An unknown provider component or an absent model key is not-found (exit 3). Failures classify through the models exit taxonomy.
...
13. Tests. Rewrite every command-invocation test to the new grammar and add coverage for the new surfaces: the bare-noun usage fault, exact-id not-found on each noun, `providers get` (known, unknown, `--models` fill), `models get` (composite hit, missing-slash usage error, unknown composite), `models list` global and `--provider`/`--agent` scoped with a filter, and confirmation that the old flat command behaviour is gone. That confirmation splits by failure mode: `agentdex get` and `agentdex list` are unknown top-level commands (exit 2); `agentdex providers` and `agentdex models <id>` no longer list, because those tokens now name noun groups — `agentdex providers` hits the bare-noun usage fault (exit 2, group help) and `agentdex models <id>` hits the unknown-verb fault (exit 2). Assert the exit code and the absence of the old listing, not a specific "unknown command" string. Follow the existing harness and prefer the frozen models.dev testdata over the network.

14. Documentation. Trim the CLI documentation in `docs/agentdex-design.md` to architecture and rationale, removing the per-command behavioural spec that duplicates the code and is the source of the drift this project corrects. Leave the design doc internally consistent with the removed behaviour, not merely re-pointed at the new grammar. The trim:

    - Replace the command list with the concise noun-verb command tree: the six data commands as `<noun> <verb>`, plus the top-level `refresh`, `version`, and `completion`. This is architecture-level reference; keep it minimal, with no per-command behaviour.
    - Remove the selector-matching section (719-742). State the two verb rules in one architecture note — `list` is a browse substring narrowing, `get` is an exact canonical-id fetch — and retire the none/one/many rule and its `providers`-exception framing entirely.
    - Remove the per-command behaviour notes (744-825). Per-command behaviour now lives in the code and `--help`; the design doc does not restate it.
    - In the catalog/models.dev coverage section, drop the two `no` rows of the rollup table and the provider-fallthrough prose (691-700), keeping the home-provider rollup rows and the reachable-vs-unreachable rationale (685-690), which is durable design intent for implemented behaviour.
    - In Output and exit codes, keep the exit-code taxonomy and remove the `get`-reporting per-case bullet that restated the coverage table and named the removed exit-2 fallthrough; an unknown agent id is exit-3 not-found with no fallthrough.
    - In Model resolution (632-636), state that `ResolveModel` remains a public library API and remove the claim that the CLI surfaces it via `models <agent> <query>`.
    - Trim the matching decisions-log bullets: the selector-matching bullet, the model-resolution CLI-surface clause, and the coverage bullet's fallthrough exit codes.
    - Fix any cross-reference the trim leaves dangling: sections that survive (for example the reachable-vs-unreachable paragraph and the provider-env reporting note) point at "See CLI get behaviour" and "see Selector matching" targets that no longer exist, so re-point or drop those references rather than leaving them hanging.

    Follow the repo markdown rules.
...
7. `models list`. Split the current `models` list path into a group-global browse: default across all providers, `--provider` and `--agent` scoping flags, the positional filter, the agnostic/home-provider rules, and validation of every caller `--provider` id in both its direct-scope and agnostic-enrichment roles (unknown id is a usage error). Preserve ordering and the price footer.

8. `models get`. Add the exact composite lookup: split on the first `/` (provider id before it, whole remainder as the model key), fetch the provider, look up the model key, render the existing single-model detail. A composite with no slash is a usage error; an unknown provider or an absent model key is not-found.
...
11. Documentation. Apply the Requirement 14 trim to `docs/agentdex-design.md`: replace the command list with the noun-verb tree, delete the selector-matching and per-command behaviour sections, drop the coverage table's fallthrough rows and prose, remove the get-reporting exit-per-case bullet, correct the Model resolution CLI-surface sentence, and trim the matching decisions-log bullets. Read the trimmed doc end to end afterwards to confirm no remaining section describes the fallthrough, the exit-2 unknown-agent code, or a CLI `models <agent> <query>` surface.
...
- A bare noun group (`agentdex agents`) exits 2; in text mode it prints the group help, and under `--json` it emits the error envelope alone on stdout (naming the missing verb) with no help text mixed in, so the output parses as JSON.
- The old flat command behaviour is gone, split by failure mode: `agentdex get <id>` and `agentdex list` fail as unknown top-level commands (exit 2); `agentdex providers` and `agentdex models <id>` no longer perform their old listing because those tokens now name noun groups — `agentdex providers` hits the bare-noun usage fault (exit 2, group help) and `agentdex models <id>` hits the unknown-verb fault (exit 2).
...
- `models list` lists across all providers; `--provider <id>` and `--agent <id>` scope it; the positional filter composes with scope; ordering is newest-first with the price footer; agnostic `--agent` without `--provider` is a usage error and a home-provider agent rejects `--provider`; an unknown `--provider` id is a usage error (exit 2) in both the direct-scope and agnostic-enrichment roles, not a silent empty listing.
...
- `docs/agentdex-design.md` shows the noun-verb command tree and states the two verb rules as a single architecture note; the none/one/many selector section, the per-command behaviour notes, the coverage table's provider-fallthrough rows, and the get-reporting exit-per-case bullet are gone; no section still describes the removed fallthrough, the exit-2 unknown-agent code, or a CLI `models <agent> <query>` surface; the home-provider coverage rollup and its rationale remain; and the decisions log matches.
