...
3. The org-tap homebrew formula completed with the real `url`, `sha256`, and `commit`, building the tagged release with `CGO_ENABLED=0`, and published to the tap repository (`start-cli/homebrew-tap`) so `brew tap start-cli/tap` serves it.
...
5. Complete the homebrew formula. With owner authorisation, fill the in-repo source-of-truth formula `dist/homebrew/agentdex.rb` (`url`, `sha256`, and `commit`) for the tagged tarball and confirm it builds agentdex with `CGO_ENABLED=0` via a local `brew install --build-from-source ./dist/homebrew/agentdex.rb`. This edits the repo copy only; the tap is not updated until step 6.
...
- The org-tap formula carries the real `url`, `sha256`, and `commit`, is published to `start-cli/homebrew-tap`, and `brew tap start-cli/tap` installs a working agentdex built with `CGO_ENABLED=0`.
