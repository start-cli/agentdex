...
2. Client
   ...
   - The fetch, decode, and merge happen once per `Client` and the merged catalog is memoised in memory; the file cache and TTL govern that single fetch, not each call. A long-lived `Client` therefore never re-merges; a refresh is picked up by a freshly constructed `Client` (the CLI's `refresh` rewrites the cache file before the next run).
   - Only a usable result is memoised. A fetch that yields a fresh catalog, or that fails but serves a stale cached copy, is memoised permanently as above. A first fetch that fails with no cache to fall back on is not memoised: the failure is returned to that caller, and the next call retries. This keeps a transient first-fetch blip (a context deadline, a momentary DNS failure) from permanently poisoning a long-lived `Client`, while the per-run detection engine is unaffected.
   - Methods are safe for concurrent use. The detection engine (document 03) calls into the client once per agent across concurrent goroutines, so the first fetch is single-flighted: concurrent callers share one in-flight fetch and one cache-file write rather than racing N requests. The shared attempt completes for all waiters; on success the memoised catalog is read-only thereafter, and on a bare failure each caller is free to retry rather than inheriting a cached error.
...
4. Two-tier validation
   ...
   - Apply the per-model required-field check only to providers a caller actually requests, inside `Provider` and `Models` — never across the full upstream catalog. A model is malformed when its `id` is empty or its `Limit` is the zero value. `Limit` stays a value type (not a pointer): an absent upstream `limit` decodes to the zero `Limit`, and a model with no usable context limit is unusable to enrich, so treating the zero value as malformed both detects absence and spares every downstream consumer a nil-check. A malformed model in an unrequested provider must not break enrichment; only a requested provider carrying a malformed model raises `ErrModelsSchema`, from the accessor that requested it.
...
- A malformed model (`id` empty or `Limit` the zero value, which an absent upstream `limit` decodes to) in an unrequested provider does not error; the same malformation in a provider passed to `Provider` or `Models` raises `ErrModelsSchema` from that accessor.
...
- Bare first-fetch failure is retryable: a first `Catalog` call that fails with no cache returns the error and is not memoised; a subsequent call against a now-reachable server fetches successfully. A success or a stale-served result, once memoised, is not re-fetched.
