...
8. Command behaviour matches this matrix:
   ...
   - `get <agnostic>` with no field filter and no `--provider`: returns all non-provider fields, omits `providers`, `provider_env`, and `models`, and adds a `warnings` entry stating the agent is provider-agnostic and how to enrich. Exit 0.
   - `get <agnostic> --fields <non-provider only>`: returns those fields, no models.dev fetch, no warning.
   - `get <agnostic> --fields models` (or `providers`, or `provider_env`) with no `--provider`: `ErrProvidersRequired`, non-zero exit.
...
5. CLI flag and field-selection seam. Add `--provider` to `get`, `models`, and `list`. Make the CLI attach the models.dev client only when the requested output needs a provider-dependent field (`providers`, `provider_env`, `models`), so a non-provider `--fields` request skips models.dev entirely. On `get`/`models`, reject `--provider` on a home-provider agent with a usage error. For agnostic agents without `--provider`: an explicit `--fields` that names any provider-dependent field is `ErrProvidersRequired`; an unfiltered `get` is the soft path — outside facts only, omit the three provider-dependent fields, warning, exit 0 — and must not call `DetectOne` with a models client attached. For `models`, pass the resolved provider set into `modelsList` and into `ResolveModel` for the query path — do not read catalog `Provider` alone for an agnostic agent. For `get` on an agnostic agent with `--provider`: do not run the catalog-data-error rollup branches; enrich through `DetectOne` with `WithProviders`/`WithModels` and surface library errors (unknown provider, `ErrProvidersRequired`) directly. Home-provider agents keep today's coverage rollup unchanged.
...
- Split get's coverage path by agent kind. Home-provider: keep `getCoverage` rollup (including "none present → catalog data error, exit 78"). Agnostic: no catalog-fault rollup; caller providers are validated in the library, and a bad or missing set is never reported as a catalog data error.
...
- `get opencode` with no `--provider` returns the outside facts, omits `providers`, `provider_env`, and `models`, and carries a provider-agnostic warning, exiting 0.
